...
> **Owner:** (repo owner)
> **Last Updated:** 2026-03-27 (Post-Session 28 — Post-Reorganization Regression)
...
- **Decimal precision is non-negotiable.** No `number` type for money or quantity in business logic. All financial arithmetic uses `Decimal.js` via `@stocker/shared` utilities. No `parseFloat()`, `Math.round()`, or native arithmetic operators (`+`, `-`, `*`, `/`) on financial values. Violations are bugs, not style issues.
...
- Multi-package interface changes (types in `@stocker/shared`)
...
|-------------|---------------|-------|
| `CORE/apps/web/prisma/schema.prisma` | Lead or designated BE teammate | Lead approval required before write |
| `CORE/packages/shared/src/types/` | Lead or designated BE teammate | Lead approval required before write |
| `CORE/apps/web/src/components/` | FE teammate | |
| `CORE/apps/web/src/app/api/` | BE teammate | |
| `CORE/packages/analytics/`, `CORE/packages/market-data/`, `CORE/packages/advisor/`, `CORE/packages/scheduler/` | BE teammate | |
| `CORE/session read-ins/CLAUDE.md`, `CORE/session read-ins/AGENTS.md`, `CORE/session read-ins/HANDOFF.md` | Lead only | Never written by teammates |
| `CORE/data/test/` | QA or BE teammate | Fixtures are shared; coordinate before write |
...
**Current baseline:** 770 passing Vitest tests across 64 files as of Session 27. Any PR that reduces this count without explicit QA sign-off is a regression.
...
| Document | Role |
|----------|------|
| `CORE/session read-ins/CLAUDE.md` | Architecture overview, coding rules, session history, agent protocols — read first every session |
| `CORE/session read-ins/AGENTS.md` | This file — tech stack, design decisions, coordination model |
| `CORE/session read-ins/HANDOFF.md` | Current project state — read at every session start, written at every session end |
| `Planning/STOCKER_MASTER-PLAN.md` | Roadmap, epics, strategic decisions — read during planning sessions |
| `CORE/SESSION-{N}-PLAN.md` | Implementation spec for session N — read only during that session |
| `CORE/SESSION-{N}-KICKOFF.md` | Launch prompt for session N — used to start that session |
| `CORE/session read-ins/KNOWN-LIMITATIONS.md` | Documented MVP gaps with impact and mitigation |
| `CORE/apps/web/prisma/schema.prisma` | Database schema — consult when touching the data layer |
| `CORE/packages/shared/src/types/` | Shared TypeScript interfaces — consult frequently |
| `CORE/data/test/reference-portfolio.json` | PnL validation fixture — used in financial testing sessions |
...
|---------|------|---------|------------|
| `@stocker/shared` | `CORE/packages/shared/` | Types, Decimal utils, ULID, constants | Nothing |
| `@stocker/analytics` | `CORE/packages/analytics/` | FIFO lots, PnL, portfolio value series | `@stocker/shared` |
| `@stocker/market-data` | `CORE/packages/market-data/` | Provider interface, implementations (FMP, Tiingo, AV, CoinGecko), calendar, rate limiter | `@stocker/shared` |
| `@stocker/advisor` | `CORE/packages/advisor/` | LLM adapter, tool definitions, system prompt, context window management | `@stocker/shared` |
| `@stocker/scheduler` | `CORE/packages/scheduler/` | Polling orchestration | `@stocker/market-data` |
| `web` | `CORE/apps/web/` | Next.js application (UI + API routes) | All packages |
