...
**Local repo path:** ~/Desktop/_LOCAL APP DEVELOPMENT/STOCKER/CORE
**GitHub:** (private repo)
...
├── session read-ins/            # Shipped project documents
...
- **No `number` type for money or quantity in business logic.** All financial arithmetic uses `Decimal.js` via `@stocker/shared` utilities.
...
```typescript
// ✅ CORRECT
import { toDecimal, mul, sub } from '@stocker/shared';
...
- Import ULID generation from `@stocker/shared`

### Rule 6: Imports and Exports

- Named exports only (no `export default` except for Next.js page/layout/route files)
- Workspace packages use `@stocker/` prefix: `@stocker/shared`, `@stocker/analytics`, `@stocker/market-data`, `@stocker/advisor`
...
| `CalendarFns` | `packages/analytics/src/value-series.ts` | `{ getNextTradingDay, isTradingDay }` from `@stocker/market-data` |

### Prisma-to-Shared Type Conversion

Prisma models return Prisma's own types. Analytics expects `@stocker/shared` types. Conversion pattern:
```typescript
