...
### AD-S28-1 — Source Code Reorganization into CORE/ Subdirectory

**Date:** 2026-03-27
**Session / Epic:** S28 — Post-Reorganization Regression
**Status:** Active

**Context:**
Post-S27, all application source code was moved from the repo root into a `CORE/` subdirectory. Planning docs, session reports, and design docs remain at the repo root. This separates operational code from project management artifacts.

**Decision:**
All source code lives under `CORE/`. Five project documents are pushed to GitHub from `CORE/session read-ins/`: AGENTS.md, CLAUDE.md, DECISIONS.md, README.md, KNOWN-LIMITATIONS.md. HANDOFF.md and PROJECT-SPEC.md are excluded via `.git/info/exclude` (operational state, too internal). CI workflow uses `working-directory: CORE`. Root `package.json`, `tsconfig.json`, `tsconfig.base.json` live at `CORE/` root (not in a config/ subdirectory).

**Rationale:**
Separation of concerns — source code that ships vs. project management artifacts that don't. KNOWN-LIMITATIONS.md is useful for contributors. HANDOFF.md contains session-specific operational state not relevant to external readers.
...
| 2026-03-27 | — | S28 additions | AD-S28-1 (source code reorganization into CORE/ subdirectory). | Lead Engineering |
